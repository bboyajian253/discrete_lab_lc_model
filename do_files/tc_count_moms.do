...
preserve
collapse (mean) log_hours_decimal if emp == 1
tempfile mean_log_hours_data
save `mean_log_hours_data', replace
restore
...
cross using `mean_log_hours_data'
...
sum job_hours if MH == 0 & age < `pre_retirement_age'
local mean_log_hours0 = r(mean)
sum job_hours if MH == 1 & age < `pre_retirement_age'
...
reg log_hours_decimal `spec' if age < `pre_retirement_age'
// store the coefficient for MH == 1
