...
foreach q in `quants'{	
	...
	*build strings for variables to keep from regressions
	local myKeepMP`q' ""
	local myKeepMH`q' ""
	local myKeepPH`q' ""
	local i = 2
	...
	***ENDS FE LOOP***
		...
		*if `qq' >= 5{
			*2.mh_Q`qq' 3.mh_Q`qq' 4.mh_Q`qq' 5.mh_Q`qq' 2.ph_Q`pq' 3.ph_Q`pq' 4.ph_Q`pq' 5.ph_Q`pq'
			esttab Reg_emp_year Reg_log_labor_inc_year Reg_log_hours_year Reg_emp_year_id Reg_log_labor_inc_year_id Reg_log_hours_year_id using reg_results_Q`q'_`spec'.txt, replace cells(b(star fmt(3)) se(par fmt(3))) ///
			...
			esttab Reg_emp_year Reg_log_labor_inc_year Reg_log_hours_year Reg_emp_year_id Reg_log_labor_inc_year_id Reg_log_hours_year_id using reg_results_Q`q'_`spec'_both.tex, booktabs replace cells(b(star fmt(3)) se(par fmt(3))) ///
...
*local savedir "$outdir/UKHLS_quant"
*cd "`savedir'"
	esttab Reg_emp_year Reg_log_labor_inc_year Reg_log_hours_year Reg_emp_year_id Reg_log_labor_inc_year_id Reg_log_hours_year_id using reg_results_`spec'_both.tex, booktabs replace cells(b(star fmt(5)) se(par fmt(3))) ///
		stats(N r2 r2_a, labels("Observations" "R-Square" "Adj. R-Square")) ///
		label /// width(\textwidth)
 		prehead( `"\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}"' ///
	...
	esttab Reg_emp_year Reg_log_labor_inc_year Reg_log_hours_year using reg_results_`spec'.tex, booktabs replace cells(b(star fmt(5)) se(par fmt(3))) ///
		stats(N r2 r2_a, labels("Observations" "R-Square" "Adj. R-Square")) ///
		label /// width(\textwidth)
 		prehead( `"\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}"' ///
		`"\center\caption{Mental and Physical Health Index Effects and Labor Outcomes}"' ///
