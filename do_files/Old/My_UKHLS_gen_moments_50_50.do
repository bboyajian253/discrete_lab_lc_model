...
global outdir   "C:/Users/Ben/My Drive/PhD/PhD Year 3/3rd Year Paper/Model/My Code/MH_Model/my_code/model_uncert/input/50_50" /*output file folder directory */
global datadir  "$curdir/Data"

local dir = "$datadir"

cd "`dir'"
use "`dir'/UKHLS_sample_for_model", clear
...
sort age
quietly sum wage if age == `start_age'
local initial_sd_wage = r(sd)

* Display the value of initial_sd_wage
display "Initial SD of wage for age `start_age': `initial_sd_wage'"

* Collapse moments by age for ages `start_age'-`end_age' and save the results
preserve
collapse (mean) job_hours_decimal if age >= `start_age' & age <= `end_age', by(age)
tempfile job_hours_data
save `job_hours_data', replace
restore

* Collapse wage and log_wage by age for ages `start_age'-`end_age' and save the results
preserve
collapse (mean) wage if age >= `start_age' & age <= `end_age', by(age)
tempfile wages_data
...
preserve
collapse (sd) wage if age >= `start_age' & age <= `end_age', by(age)
rename wage sd_wage
...
gen MH_cluster = .
replace MH_cluster = 0 if MH_perc < 50
replace MH_cluster = 1 if MH_perc >= 50
tempfile clust_data
save `clust_data'
restore
...
foreach clust in `MH_clusters'{
	foreach tran in `trans' {
		sum MH_`tran' if `start_age' <= age & age <= `end_age' & MH_cluster == `clust'
		local MH_`tran'_mean = r(mean)
...
forvalues i = 1/2{
	foreach tran in `trans'{
		...
		tab age MH_`tran' [aweight=wght0] if `start_age' <= age & age <= `end_age' & MH_cluster == `MH_clus_ind', matcell(xx) matrow(yy) matcol(zz)
...
tab age MH_P2G [aweight=wght0] if `start_age' <= age & age<=`end_age' & MH_cluster == 0, matcell(xx) matrow(yy) matcol(zz)
mata : xx = st_matrix("xx")
mata : xx = xx :/rowsum(xx)
mata : st_matrix("xx",xx)	
...
*1-year binary health state transitions
tab age MH_P2G [aweight=wght0] if `start_age' <= age & age<=`end_age' & MH_cluster == 1, matcell(xx) matrow(yy) matcol(zz)
mata : xx = st_matrix("xx")
mata : xx = xx :/rowsum(xx)
mata : st_matrix("xx",xx)	
putexcel set "$outdir/MH_trans", sheet("Figure1_MH") modify	
...
*1-year binary health state transitions
tab age MH_G2P [aweight=wght0] if `start_age' <= age & age<=`end_age' & MH_cluster == 0, matcell(xx) matrow(yy) matcol(zz)
mata : xx = st_matrix("xx")
mata : xx = xx :/rowsum(xx)
mata : st_matrix("xx",xx)	
putexcel set "$outdir/MH_trans", sheet("Figure1_MH") modify	
...
*1-year binary health state transitions
tab age MH_G2P [aweight=wght0] if `start_age' <= age & age<=`end_age' & MH_cluster == 1, matcell(xx) matrow(yy) matcol(zz)
mata : xx = st_matrix("xx")
mata : xx = xx :/rowsum(xx)
mata : st_matrix("xx",xx)	
putexcel set "$outdir/MH_trans", sheet("Figure1_MH") modify	
