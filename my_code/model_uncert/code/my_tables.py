...
def print_endog_params_to_tex(myPars: Pars, targ_moments: Dict[str, float], model_moments: Dict[str, float], path: str = None) -> None:
    ...
    tab = [
        ...
        f"$\\mu_{{w_{{0,\gamma}}}}$ &  Lab. FE mean & {round(myPars.lab_fe_tauch_mu, 4)} & Mean wage, $j=0$ & {round(w0_mu_targ_val,4)} & {round(w0_mu_mod_val,4)} \\\\ \n",
        f"$\\sigma_{{w_{{0,\gamma}}}}$ & Lab. FE SD & {round(myPars.lab_fe_tauch_sigma, 4)} & SD wage, $j=0$ & {round(w0_sigma_targ_val,4)} & {round(w0_sigma_mod_val,4)} \\\\ \n",
