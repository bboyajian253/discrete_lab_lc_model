...
def print_endog_params_to_tex(myPars: Pars, targ_moments: Dict[str, float], model_moments: Dict[str, float], path: str = None) -> None:
    ...
    tab = [
        ...
        f"$\\mu_{{\gamma}}$ & Labor prod. mean & {round(myPars.lab_fe_tauch_mu, 4)} & Mean wage, $j=25$ & {round(w0_mu_targ_val,4)} & {round(w0_mu_mod_val,4)} \\\\ \n",
        f"$\\sigma_{{\gamma}}$ & Labor prod. SD & {round(myPars.lab_fe_tauch_sigma, 4)} & SD wage, $j=25$ & {round(w0_sigma_targ_val,4)} & {round(w0_sigma_mod_val,4)} \\\\ \n",
...
def print_exog_params_to_tex(myPars: Pars, path: str = None)-> None:
    ...
    tab = ["\\documentclass[border=3mm,preview]{standalone}",
            ...
            # f"$\\omega_{{H=0}}$ & Low type pop. weight & {np.round(myPars.H_type_perm_weights[0], 4)} & Benchmark \\\\ \n",
            # f"$\\omega_{{H=1}}$ & High type pop. weight & {np.round(myPars.H_type_perm_weights[1], 4)} & Benchmark \\\\ \n",
            "\\hline \n",
            "\\end{tabular}\n",
            "\\end{document}\n"
            ]
