...
class est_oop(ddcpmodel):
    ...
    def ComputeMoments(self,df=10):
        ''' compute a bunch of moments from data frame df
            if df is an integer, generate the moments
        '''
    ...
    def LossFunction(self,pars) :
        ...
        dd = est_oop(simulate=1,print_screen=self.print_screen,**pars) #already have real moments, so just simulate
 ...
 #       myformat= '{:7.5f} loss: {:10.5f}'
        if self.print_screen >= 1 :
#            print(myformat.format(pars['w_intercept'],loss))
            print(pars,loss)
        return loss
...
result = minimize(aa.OptFunction, x0= blowup(10.3,bottom,top), #x0 is thes starting value
                  args=(bottom, top), 
                  method= 'L-BFGS-B', #nelder-mead', 
                  options={'disp':True})
print('x:  ',squeeze(result.x,bottom,top))
print('Minimization convergence in {:5.3f} seconds'.format(time.process_time() - start_time))
...
bottom1 = 0.01
top1 = 0.09
x0 = [blowup(10.3,bottom,top),blowup(0.04,bottom1,top1)]
#x0 = [blowup(10.06,bottom,top),blowup(0.045,bottom1,top1)]

start_time = time.process_time()
result = minimize(aa.OptFunction2, x0, 
                  args = (bottom, top, bottom1, top1),
...
# result = minimize(aa.UnconstrOpt2,[10.3,0.2],method='nelder-mead',options={'disp':True})
# print(result.x)
