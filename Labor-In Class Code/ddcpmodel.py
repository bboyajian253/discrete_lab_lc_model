...
@author: Andrea Moro, partially ported and adapted from Fortran code by Meghan Skira
"""
...
class ddcpmodel() :
    ...
    def __init__(self,     
        ...
        self._maxexp = nt+1 #max value of experience

        #value function for all states; 0=time, 1=experience, 2=edu, 3=last work, 4=married
        self._VF = np.zeros((self.nt+1,self._maxexp,3,2,2))  
        
        # if class is ddcpmodel, then compute the value function
        if self.__class__.__name__ == 'ddcpmodel' :
            self.ValueFunction()
    ...
    def _EV_Offer(self,time,x,edu,lw,mar,wshvec,ushvec) :
        ...
        if time == self.nt :
            ...
            for draw in range(self.dt) :
                sum = sum + self._valueOffer(time,x,edu,lw,mar,wshvec[draw],ushvec[draw])
            return sum/self.dt
    ...
    def ValueFunction(self) :
        ...
        for t in reversed(range(self.nt+1)) :  # start from last period
            if self.print_screen>=2 :
                if t == self.nt :
                    print('Computing value function for period',t,'... ',end="",flush=True)
                elif t>0 :
                        ...
                        for mar in range (2) : # married 0,1
                            if xper<=t :  
                                if lw == 1 and t < self.nt : # receives offer for sure
                                    self._VF[t,xper,ed,lw,mar] = self._EV_Offer(t,xper,ed,lw,mar,wsh[:,t],ush[:,t]) 
                                elif t<self.nt : # may receive offer
                                    self._VF[t,xper,ed,lw,mar] = self.offerProb(ed,mar) * self._EV_Offer(t,xper,ed,lw,mar,wsh[:,t],ush[:,t]) + \
                                        (1-self.offerProb(ed,mar)) * self._EV_NoOffer(t,xper,ed,lw,mar,wsh[:,t],ush[:,t])                              
                                else : # last period
                                    self._VF[t,xper,ed,lw,mar] = self._EV_NoOffer(t,xper,ed,lw,mar,wsh[:,t],ush[:,t]) 
        
        return (self._VF,wsh,ush)
    ...
    def many_histories(self,rep=1,drawshock=1,ifile='InitData_BasicDPP_1.txt', 
        ...
        for ob in observations :
            ...
            for reps in range(rep) :
                ...
                # compute a history
                hist = self.history(ob['edu'], ob['mar'], drawshock=1) #0,ushock[n,:],wshock[n,:],ofshock[n,:]))
    ...
    def saveData(self,df=10,filename='simdata.csv') : 
        ''' saves simulated data to csv file , computes histories if not provided
            df       : data frame with histories, or integer = n. of replications to generate it
            filename : name of the csv file
        '''
        
        if type(df) == int :
            df = self.many_histories(rep=df)

        df.to_csv(filename)    
    
    def stats(self,df=10,level=['time']) :
        """ returns some summary statistics
            df: data frame with histories, or integer = n. of replications to generate it
            level: what statistics to report (time, edu, exp, or reg)
        ...
        if 'time' in level :  
            ...
            for t in range(self.nt) :
                print(myformat.format(t
                    ...
                    ,df["owage"][df['offer']==1][df['time']==t].mean()        
                    )
                )
            print()
        ...
        if 'reg' in level :
            ...
            # copy is needed here to avoid Python's SettingWithCopyWarning
            dfwork = df.loc[df['work']==1].copy()
...
if __name__ == "__main__":
    ...
    d2 = ddcpmodel(dt=2500, nt= 5)
    d3 = ddcpmodel(dt=2500,u_work=-2, nt=5)
 
    # generate simulated histories
    hist2 = d2.many_histories(rep=10)
    hist3 = d3.many_histories(rep=10)
    
    d2.stats(hist2)
    d3.stats(hist3)

    d2.saveData(hist2,filename='simdata.csv')
    d2.stats(hist2,level='reg')


#%%
    d4 = ddcpmodel(dt=2500, nt= 15)
